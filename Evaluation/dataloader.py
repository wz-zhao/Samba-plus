...
class EvalDataset(data.Dataset):
    def __init__(self, pred_root, label_root, task):
        ...
        if task == "VSOD":
            ...
            for file in files:
                root = os.path.join(label_root, file)
                imgs = os.listdir(root)
                for img in imgs:
                    pred_names.append(os.path.join(file, img.replace(file+'_','')))
                    gt_names.append(os.path.join(file, img))
            ...
            for file in files:
                root = os.path.join(pred_root, file)
                imgs = os.listdir(root)
                for img in imgs:
            ...
            pred_names = os.listdir(label_root)
            gt_names = pred_names
