...
class Dataset(data.Dataset):
    def __init__(self, datasets, mode='train', transform=None, return_size=True):
        ...
        for (i, dataset) in enumerate(datasets):
            ...
            if mode == 'train':
                data_dir = './dvsod_dataset/{}'.format(dataset)
            ...
            for line in imgset_file:
                ...
                if dataset!="DVisal" and "PTB" in img_path or "rgbd" in img_path :
                    data['img_path'] = "./dvsod_dataset/DVisal" + img_path
                    data['gt_path'] = "./dvsod_dataset/DVisal" + gt_path
                    data['flow_path'] = "./dvsod_dataset/DVisal" + line.strip("\n").split(" ")[2]
                    data['depth_path'] = "./dvsod_dataset/DVisal" + line.strip("\n").split(" ")[3]
                exclude_keys = ["PTB", "CDTB", "tracklam", "Track-it-in-3D", "rgbd", "DET"]
                if dataset == "DVisal" and all(key not in img_path for key in exclude_keys):
                    data['img_path'] = "./dvsod_dataset/vidsod_100" + img_path
                    data['gt_path'] = "./dvsod_dataset/vidsod_100" + gt_path
                    data['flow_path'] = "./dvsod_dataset/vidsod_100" + line.strip("\n").split(" ")[2]
                    data['depth_path'] = "./dvsod_dataset/vidsod_100" + line.strip("\n").split(" ")[3]
