...
from models.encoders.vmamba import CVSSDecoderBlock, AlignedMambaBlock
...
class AlignedUpsample(nn.Module):
    def __init__(self, input_resolution, dim, dim_scale=2, norm_layer=nn.LayerNorm):
        ...
        #     norm_layer(dim),
        # )
        self.aligned_mamba = AlignedMambaBlock(
                hidden_dim= dim,
                mlp_ratio=0.0,
                d_state=4
                )
        # self.expand1 = nn.Linear(dim//2, dim, bias=False)
        self.expand2 = nn.Linear(dim, dim*2, bias=False)
...
class Mamba_up(nn.Module):
    def __init__(self, dim, input_resolution, depth, dt_rank="auto", 
        ...
        if upsample is not None:
            ...
            self.upsample = AlignedUpsample(input_resolution, dim=dim, dim_scale=2, norm_layer=norm_layer)
        else:
            self.upsample = None
        ...
        for i_layer in range(self.num_layers):
            if i_layer == 0:
                ...
                layer_up = Mamba_up(dim=int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)),
                                         ...
                                         norm_layer=norm_layer,
                                         upsample=AlignedUpsample if (i_layer < self.num_layers - 1) else None,
                                         use_checkpoint=use_checkpoint)
    ...
    def forward_up_features(self, inputs):  # B, C, H, W
        if not self.deep_supervision:
            for inx, layer_up in enumerate(self.layers_up):
                if inx == 0:
                    x1 = inputs[3 - inx]  # B, 768, 14, 14
                    x2 = inputs[3 - inx -1]  # B, 768, 28, 28
                    x1 = x1.permute(0, 2, 3, 1).contiguous()  # B, 14, 14, 768
                    x2 = x2.permute(0, 2, 3, 1).contiguous()  # B, 28, 28, 768
                    y = layer_up(x1, x2)  # B, 30, 40, 384
                else:
                    ...
                    x1 = y + inputs[3 - inx].permute(0, 2, 3, 1).contiguous()
                    x2 = inputs[3 - inx -1].permute(0, 2, 3, 1).contiguous()
                    y = layer_up(x1, x2)
