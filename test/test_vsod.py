...
parser.add_argument('--model_path', type=str, default='./checkpoints/Samba/vsod_datasets/Samba_vsod.pth')
parser.add_argument('--test_dataset', type=list, default=['DAVIS','DAVSOD','FBMS','SegTrack-V2','VOS'])
...
for i, data_batch in enumerate(test_loader):
    ...
    image, flow, name, split, size = data_batch['image'], data_batch['flow'], \
    ...
    with torch.no_grad():
        ...
        for i in range(config.test_batch_size):
            presavefold = os.path.join(config.testsavefold, dataset[i],'Mamba', split[i])
