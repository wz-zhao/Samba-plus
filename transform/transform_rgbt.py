...
class Normalize(object):
    ...
    def __call__(self, sample):
        ...
        depth = np.array(sample['thermal']).astype(np.float32)
        img /= 255.0
        img -= self.rgb_mean
        img /= self.rgb_std

        depth /= 255.0
        depth -= self.thermal_mean
        depth /= self.thermal_std
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        depth = np.array(sample['thermal']).astype(np.float32).transpose((2, 0, 1))
