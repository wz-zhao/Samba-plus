...
class Normalize(object):
    ...
    def __init__(self, rgb_mean, rgb_std, flow_mean, flow_std):
        self.rgb_mean = np.array(rgb_mean, dtype=np.float32)
        self.rgb_std = np.array(rgb_std, dtype=np.float32)
        self.flow_mean = np.array(flow_mean, dtype=np.float32)
        self.flow_std = np.array(flow_std, dtype=np.float32)
...
class RandomFlip(object):
    def __call__(self, sample):
        ...
        #     flow = flow.transpose(Image.FLIP_TOP_BOTTOM)
